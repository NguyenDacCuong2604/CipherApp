...
public class SymmetricEncryptionGUI extends JFrame {
    ...
    private void renderDecryption(JPanel panel, AbsSymmetricEncryption absSymmetricEncryption) {
        ...
        if (!absSymmetricEncryptionEncrypt.name.equals(Constants.Cipher.HILL) || !absSymmetricEncryption.name.equals(Constants.Cipher.VIGENERE)) {
            ...
            nameFileLabel.setPreferredSize(new Dimension(300, 40));
            filePanel.add(nameFileLabel);
            filePanel.setVisible(false);
            decryptionPanel.add(filePanel);
            //select mode
            ...
            decryptButton.addActionListener(e -> {
                if(!isFileSelected) {
                    ...
                    if (fileDecryption==null) {
                        ...
                        String cipherText;
                        absSymmetricEncryption.instance(absSymmetricEncryption.name+"/"+ (String)modeComboBox.getSelectedItem());
                        absSymmetricEncryption.convertKey(keyTextArea.getText());
                        absSymmetricEncryption.convertIv(ivTextArea.getText());
                        cipherText = absSymmetricEncryption.decryptFile(fileDecryption);
                        outTextArea.setText(cipherText);
                    }
                }
            });
            ...
            textRadioButton.addItemListener(e -> {
                if (e.getStateChange() == ItemEvent.SELECTED) {
                    ...
                    filePanel.setVisible(false);
                    replaceComboBoxValues(modeComboBox, this.methods);
    ...
    private void replaceComboBoxValues(JComboBox comboBox, String[] listItem) {
        // Xóa tất cả các mục hiện tại trong combobox
        comboBox.removeAllItems();

        // Thêm các giá trị mới từ mảng mới vào combobox
        for (String newItem : listItem) {
