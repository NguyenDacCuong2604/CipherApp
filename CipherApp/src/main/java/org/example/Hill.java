...
public class Hill {
    ...
    //main method
    public static String decryptHillCipher(String ciphertext, int[][] keyMatrix, char[] alphabet) {
        int n = keyMatrix.length;
        int mod = alphabet.length;
        StringBuilder plaintext = new StringBuilder();

        for (int i = 0; i < ciphertext.length(); i += n) {
            char[] segment = ciphertext.substring(i, i + n).toCharArray();
            int[] encryptedSegment = new int[n];

            for (int j = 0; j < n; j++) {
                for (int k = 0; k < n; k++) {
                    encryptedSegment[j] += (segment[k] - alphabet[0]) * keyMatrix[k][j];
                }
                encryptedSegment[j] = (encryptedSegment[j] % mod + mod) % mod;
            }

            for (int j = 0; j < n; j++) {
                plaintext.append(alphabet[encryptedSegment[j]]);
            }
        }
    ...
    public static void main(String[] args) {
        char[] alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".toCharArray(); // Chữ cái tiếng Anh
        int[][] keyMatrix = {{15, 14, 11}, {13, 19, 15}, {21, 2, 16}}; // Ma trận khóa
        String ciphertext = "DACCUONGG"; // Ví dụ mật mã đã mã hóa

        String plaintext = decryptHillCipher(ciphertext, keyMatrix, alphabet);
